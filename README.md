...
وفيه أيضًا **خادم + لوحة تحكم**: ترفع المقاطع من المتصفح فتظهر فورًا لكل من يفتح الرابط،
و**APK واحد للهاتف وتلفاز أندرويد** يتصل بخادمك.
**[⬇ تحميل LiwaTube.apk](https://github.com/almarar88/liwamusic/releases/download/tube-android-v1.0.0/LiwaTube.apk)** · البناء: `build-liwatube.yml` (ويندوز، وسوم `tube-v*`) و `build-liwatube-android.yml` (APK، وسوم `tube-android-v*`).
...
**[⬇ تحميل LiwaMusic.apk](https://github.com/almarar88/liwamusic/releases/download/android-v1.2.0/LiwaMusic.apk)**
· [كل إصدارات الأندرويد](https://github.com/almarar88/liwamusic/releases?q=android-v)

**روابط بديلة** (إن تعذّر التحميل من الرابط الأول — بعض الشبكات تحجب خادم
مرفقات جيت هَب، والملف نفسه بالضبط):

| المرآة | الرابط |
| --- | --- |
| jsDelivr (شبكة توزيع عالمية) | <https://cdn.jsdelivr.net/gh/almarar88/liwamusic@main/download/LiwaMusic.apk> |
| raw.githubusercontent | <https://raw.githubusercontent.com/almarar88/liwamusic/main/download/LiwaMusic.apk> |

> بصمة الملف للتحقّق: `MD5 32b18681d2cd0e5ece7e7faa68e886bf` — الحجم 3,041,987 بايت.

### إذا «رفض التحميل» أو «فشل التثبيت»

| ما تراه | السبب | الحل |
| --- | --- | --- |
| كروم يقول «قد يضر هذا الملف بجهازك» أو يوقف التنزيل | كروم يحذّر من كل ملفات APK | من شريط التنزيلات: **⋮ ← تنزيل على أي حال** (Download anyway) |
| لا يبدأ التنزيل أصلًا داخل واتساب/تيليجرام/تويتر | المتصفّح المدمج في التطبيقات لا ينزّل APK | افتح الرابط في **كروم** أو أي متصفّح مستقل |
| التنزيل يتوقّف في المنتصف أو «فشل الشبكة» | حجب/بطء على `objects.githubusercontent.com` | استخدم رابط **jsDelivr** أعلاه |
| «لأسباب أمنية، هاتفك غير مسموح له بتثبيت تطبيقات من هذا المصدر» | تثبيت من خارج المتجر معطّل | **الإعدادات ← التطبيقات ← وصول خاص ← تثبيت تطبيقات غير معروفة** ← اختر كروم ← فعّل |
| «لم يتم تثبيت التطبيق» (App not installed) | نسخة قديمة بتوقيع مختلف، أو ملف ناقص | أزل النسخة القديمة ثم ثبّت، وتأكّد أن حجم الملف 3,041,987 بايت |
| «هذا التطبيق غير متوافق» | أندرويد أقدم من 5.1 | التطبيق يتطلّب **أندرويد 5.1 (API 22)** فأحدث |
...
واجهة زجاجية (Glassmorphism) بخلفية بنفسجية دافئة: بطاقات مموّهة الخلفية، شريط
تنقّل عائم، حلقات تقدّم، وبطاقات بطل عريضة — والألوان **تُستخرج من غلاف الأغنية
الجارية** فتتبدّل الخلفية والبطاقات مع كل مقطع.

<p align="center">
  <img src="docs/m-03-home-playing.png" width="212" alt="الرئيسية" />
  <img src="docs/m-08-activity.png" width="212" alt="النشاط" />
  <img src="docs/m-04-player.png" width="212" alt="المشغّل" />
</p>

<p align="center">
  <img src="docs/m-06-immersive.png" width="212" alt="الوضع الغامر" />
  <img src="docs/m-09-albums.png" width="212" alt="الألبومات" />
  <img src="docs/m-12-settings.png" width="212" alt="الإعدادات" />
</p>
...
- **حفظ للاستماع دون إنترنت** لأغنية واحدة أو لقائمة أو للمكتبة كلها.
...
- خلط، تكرار (الكل/أغنية)، سرعة تشغيل (0.75× إلى 1.75×)، مفضلة، تقييم بالنجوم.
- **قائمة انتظار** حيّة: شغّل بعد الحالية، أضف لآخر الطابور، احذف من الطابور.
- **قوائم تشغيل** تُنشأ وتُحذف من الهاتف وتتزامن مع الكمبيوتر.
- تصفّح بالألبوم أو **الفنان أو النوع أو المجلد**.
- بحث فوري واعٍ بالعربية (يتجاهل التشكيل ويوحّد الألف والياء والتاء المربوطة).

**جديد في 1.2**
- **كلمات متزامنة**: يبحث التطبيق عن ملف `.lrc` (أو `.txt`) بنفس اسم الأغنية في
  درايف، ويُبرز السطر الجاري ويقفز للثانية عند لمس أي سطر.
- **وضع غامر** بملء الشاشة: غلاف كبير وكلمات متتابعة على خلفية مأخوذة من الغلاف.
- **معادل صوتي بعشرة نطاقات** مع أنماط جاهزة (باص قوي، صوت واضح، طرب شرقي،
  ليلي، صاخب) و**تسوية مستوى الصوت** بين المقاطع.
- **تلاشٍ تدريجي** بين المقاطع (حتى 10 ثوانٍ).
- **شاشة نشاط**: هدف استماع يومي، حلقات تقدّم لأيام الأسبوع، سلسلة الأيام
  المتتالية، مجموع دقائق الأسبوع، أكثر فنان وأكثر ألبوم، وجلسات اليوم بنسبة
  اكتمال كل مقطع.
- **مؤقت نوم** بخيار «توقّف بعد نهاية المقطع».
- **مركز تنبيهات** داخل التطبيق (فهرسة، حفظ، بلوغ الهدف، مزامنة).
- **ألوان ديناميكية** مستخرجة من غلاف الأغنية تُلوّن الخلفية والبطاقات.
- **رفوف مقترحات** في الرئيسية: تابع الاستماع، الأكثر تشغيلًا، لم تسمعها بعد.

**المزامنة**
- **مزامنة تلقائية** مع الكمبيوتر عند فتح التطبيق وعند الضغط على زر المزامنة —
  تشمل المفضلة والتقييمات وعدّادات التشغيل والقوائم ويوميات الاستماع.
