...
صفحة تعريفية للتطبيق في مجلد [`site/`](site/) — HTML وCSS وJavaScript بلا خطوة
بناء، بحركات تمرير عبر GSAP. تُنشر تلقائيًا على GitHub Pages عند أي تعديل في
`site/` على `main` عبر سير العمل «نشر موقع LiwaMusic» (فعّل Pages من
**Settings ← Pages ← Source: GitHub Actions** مرة واحدة). للمعاينة محليًا:

```bash
npx serve site   # أو: python3 -m http.server -d site 8080
```
